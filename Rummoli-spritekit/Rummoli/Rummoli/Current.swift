...
//  Rummoli
//
//  Created by Scott Mayhew on 2015-09-24.
//  Copyright © 2015 Huge Holdings Inc. All rights reserved.
//

import SpriteKit

class Current: SKScene {
    ...
    func playcard(player player: Int) {
        ...
        if player == 2 {
                ...
                for card in player2Cards {
                    ...
                    if card.suit == playedCard[playedCard.count - 1].suit {
                        if card.rank == playedCard[playedCard.count - 1].rank + 1 {
                            ...
                            if switchToRed {
                                if card.suit == "Diamonds" && card.rank == 14 {
                                    player2PlayingCard.append(card)
                                    switchToRed = false
                                    switchToBlack = true
                                    break
                                }
                                if card.suit == "Diamonds" && card.rank == 13 {
                                    player2PlayingCard.append(card)
                                    break
                                }
                                if card.suit == "Diamonds" && card.rank == 12 {
                                    player2PlayingCard.append(card)
                                    break
                                }
                                if card.suit == "Diamonds" && card.rank == 11 {
                                    player2PlayingCard.append(card)
                                    break
                                }
                            ...
                            if switchToBlack {
                                if card.suit == "Spades" && card.rank == 14 {
                                    player2PlayingCard.append(card)
                                    switchToRed = true
                                    switchToBlack = false
                                    break
                                ...
                                if card.suit == "Spades" && card.rank == 11 {
                                    player2PlayingCard.append(card)
                                    break
                                }
            ...
            moveCard(node: player2PlayingCard[move], player: self.currentPlayer)
            move++
        }
    }
                        ...
                        if playedCard.count == 0 {
                            ...
                            if switchToRed == false {
                                if node.suit == playedCard[playedCard.count - 1].suit {
                                    if node.suit == "Spades" && node.rank == 14 {
                                        switchToRed = true
                                        switchToBlack = false
                                    }
                                    moveCard(node: node, player: self.currentPlayer)
